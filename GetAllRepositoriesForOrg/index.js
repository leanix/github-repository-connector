...
class GetAllRepositoriesForOrgHandler {
	...
	async getAllRepositoryIds(orgName, repoNamesFilterListChecked, repoNamesFilterStrategy) {
		...
		if (!finalResult || !finalResult.length) {
			...
			throw new Error(
				`Zero repositories found in ${orgName} GitHub organisation. Hint: In case organisation is valid, check if the inclusion list has at least one valid repository name. or select the "Exclude" strategy with empty list to allow all repositories.`
			);
		}
